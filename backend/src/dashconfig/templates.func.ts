...
import { Widget, TemplatWidgeteType, LayoutWidgetType, ControlWidetType, MiscWidgetType, isTemplateWidget, isLayoutWidget, Card, LayoutWidget } from "../model/dash.model";

type templateResolver = (item: Widget, parentValueMap: any) => Widget | Widget[];

const TemplateRegex = new RegExp("{{([^}}]+)}}", 'gi');

const resolvers: { [index in TemplatWidgeteType | LayoutWidgetType | ControlWidetType | MiscWidgetType]?: templateResolver } = {
...
function templateReplace(str: string, valueMap: any): string {
    ...
    const result = str.replace(TemplateRegex, function (matched, group) {
        console.log(`matched: ${matched} - ${group}`);
        return Object.prototype.hasOwnProperty.call(valueMap, group) ? valueMap[group] : "";
    });
    console.log(`input [${str}] -- output: [${result}] -- valuemap: `, valueMap)
    return result;
}
        ...
        const tmpl = widget.valuesList.flatMap(valueMap => {
            ...
            return widget.template.map(item => {
                ...
                if (isLayoutWidget(resolvedItem)) {
                    console.log(`Traversing into items of ${resolvedItem.type}`);
                    resolvedItem.items = resolveTemplates(resolvedItem.items, values) as Widget[];
                }
                return resolvedItem;
            })
        })
        console.log('Templated: ', tmpl)
        return tmpl;
    }
    return [widget];
}
